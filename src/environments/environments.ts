export const environment = {
    production: false,
    KEY_USERNAME: 'exam.username',
    freeTranslateNumber: 3,
    KEY_NUMBEROFTRANSLATES: 'exam.numberoftranslates',
    GOOGLE_API_KEY:'',
    GOOGLE_API_URL:'https://translation.googleapis.com/language/translate/v2/',
    // ====================== 
    APP_NAME: 'Covid alkalmazás',
    freeCovidSearch: 3,
    KEY_NUMBEROFCOVIDSEARCH: 'exam.numberofcovidsearch',
    coivdCountrys:['Hungary', 'France', 'Slovakia', 'Slovenia', 'Austria', 'Romania'],
    COVID_API_URL:'https://europe-central2-webuni-js-covid-exam.cloudfunctions.net/'
};